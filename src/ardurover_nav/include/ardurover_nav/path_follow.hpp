...
// Carrot and heading used for one Control() tick. `bend` is 0..1 look-ahead tightness.
struct Pursuit {
    Waypoint target{};
...
inline Pursuit pursuit_target(
    ...
    if (turning) {
        ...
        out.bend = std::clamp(std::abs(out.heading_error) / kTurnHeadingRad, 0.0, 1.0);
        out.lookahead_m = kLookaheadM + (kLookaheadMinM - kLookaheadM) * out.bend;
        if (out.bend > 0.0) {
            out.target = lookahead_waypoint(path, track_i, track_t, out.lookahead_m);
            out.heading_error = heading_toward(pose, out.target);
        }
...
inline double tracking_speed(
    ...
    const double bend_scale = turning ? 1.0 : 1.0 - (1.0 - kMinTurnSpeed) * bend;
    double speed = turning ? kTurnCreep : kCruiseSpeed * end_scale * bend_scale;
